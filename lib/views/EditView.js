...
const Editor = (props) => {
  ...
  // const updateState = () =>
  //   onChange({ [name]: editor.getText() })

  const onDidAttach = (ref) => {
    if (!ref || editor)
      return
    editor = ref.getModel()
    editor.setText(value)
    // editor.onDidStopChanging(updateState)
  }

  return (
    <atom-text-editor mini name={name} ref={onDidAttach} />
  )
}
...
export default class ToolBarEditPanelView extends BaseView {
  ...
  updateState (props={}) {
    this.setState(props)
    console.info("state changed to", this.state, "(", props, ")")
  }
